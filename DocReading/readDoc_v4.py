...
def convert_image(path):
    ...
    total_time = datetime(2018,1,1,0,0,0)
...
def find_health(text):
    ...
    info = re.compile('Natureofcuts:(.*)Typeofpackaging:CARTONSNumberofcutsorpackages:(.*)Netweight:(.*),000Dateorperiodofproduction:(.*)Productbrand:(.*)Shippingmark:')
    health = health.findall(text)
    keys = ['name','cartons','weight','pdtndate','brand']
    compiled = [dict(zip(keys,info.findall(x)[0])) for x in health]    
    #try different regex 
    if len(health)==0:
        health = re.compile('IDENTIFICATIONOFPRODUCT.*?DESTINATIONOFPRODUCT')
        info = re.compile('Descriptionofproduct:(.*)\(b\)Typeandnumberofpackages:CARTONS,(.*)\(c\)Netweightofconsignment:(.*),000Kg\(d\).*Dateofslaughter:(.*)Dateofprocessing:')
        health = health.findall(text)
        keys = ['name','cartons','weight','pdtndate']
        compiled = [dict(zip(keys,info.findall(x)[0])) for x in health]              
    return compiled
